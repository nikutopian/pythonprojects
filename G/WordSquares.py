...
class Solution(object):
    def dosomething(self, wordslistsofar, word, index):
        if self.getwordspossible(word, index):
            if index < len(wordslistsofar):
                wordslistsofar[index] = word
            if index >= len(wordslistsofar) - 1:
                yield wordslistsofar[:]
            else:
                prefixlist = []
                for i in range(index + 1):
                    prefixlist.append(wordslistsofar[i][index + 1])
                prefix = ''.join(prefixlist)
                prefixwords = [word for word in self.words if word.startswith(prefix)]
                for prefixword in prefixwords:
                    for item in self.dosomething(wordslistsofar, prefixword, index + 1):
                        yield item

    def getwordspossible(self, word, index):
        if index >= len(word) - 1:
            return True
        for i in range(index + 1, len(word)):
            c = word[i]
            if c not in self.lettermappings[index]:
                return False
    ...
    def wordSquares(self, words):
        ...
        for i in range(n):
            lettermapping = defaultdict(list)
            for index, word in enumerate(words):
                c = word[i]
                lettermapping[c].append(index)
            self.lettermappings.append(lettermapping)

        itemlist = []
        for word in words:
            for item in self.dosomething([''] * n, word, 0):
