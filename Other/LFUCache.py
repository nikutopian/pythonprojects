...
def printdll(dll):
    ...
    while node is not None:
        print('{0} : {1}'.format(node.key, node.val))
        node = node.next
    print("------")


def printcache(cache):
    # for key,node in cache.keynodemap.items():
    #     print('{0} - {1} - {2}'.format(key, node.val, node.count))
    # print("---------")
    for key,dll in cache.freqdllmap.items():
        print(key)
        printdll(dll)
    print("XXXXXXXXX")
...
class LFUCache(object):
    ...
    def updatenode(self, node):
        if node.count in self.freqdllmap:
            self.freqdllmap[node.count].deletenode(node)
            if self.freqdllmap[node.count].count == 0:
                del self.freqdllmap[node.count]
        node.incrementcount()
        if node.count not in self.freqdllmap:
            self.freqdllmap[node.count] = DLL()
        self.freqdllmap[node.count].insert(node)
    ...
    def put(self, key, value):
        ...
        if self.count == self.capacity:
            ...
            if len(self.freqdllmap.keys()) > 0:
                minfrequency = min(self.freqdllmap.keys())
                topnode = self.freqdllmap[minfrequency].pop()
                if topnode.key in self.keynodemap:
                    del self.keynodemap[topnode.key]
                self.count -= 1
            else:
                return
